...
export default function ({ expenseData, handleHover, handleLeave, hoveredId, deleteExpense, editExpenseBox, inputExpense, removeExpenseBox, handleEditedInput }) {
    // Expense elements 
        ...
        return (
            <div key={data.id}
                ...
                <div className="flex gap-5 items-center">
                    {/* Edit button */}
                    <button onClick={() => editExpenseBox(data.id)}
    ...
    return (
        <>
            {/* Edit expense */}
            {inputExpense === "edit" && <InputExpense event={"edit"} removeExpenseBox={removeExpenseBox} handleEditedInput={handleEditedInput} />}
            {expenseElments}
        </>
    )
}